...
The Supabase URL and publishable/anon key are designed for browser use. `DEEPSEEK_API_KEY` and the Azure Document Intelligence variables are server-only: do not give them a `NEXT_PUBLIC_` prefix. Do not add `SUPABASE_SERVICE_ROLE_KEY`, a database password, or the Google OAuth client secret.
...
- [ ] `AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT` and `AZURE_DOCUMENT_INTELLIGENCE_KEY` are set only in Vercel server environments, Azure receipt-scanning consent is tested from Settings, and a JPEG/PNG scan is reviewed before saving.
