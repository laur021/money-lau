...
function mapProfile(profile: ProfileRow): SalaryProfile {
  return {
    ...
    basePay: Number(profile.base_pay),
...
export async function getSalaryProfiles({ includeArchived = false } = {}) {
  ...
  let query = supabase
    ...
    .select(
      "id,name,employer_name,job_title,currency,pay_frequency,base_pay,default_account_id,default_income_category_id,is_archived,salary_profile_components(id,name,component_kind,calculation_type,fixed_amount,percentage,hours,hourly_rate,multiplier,display_order)",
    )
    .order("is_archived")
    .order("name");
...
export async function getSalaryRuns(limit = 100) {
  ...
  const { data, error } = await supabase
    .from("salary_runs")
    .select(
      "id,profile_id,profile_name,employer_name,job_title,currency,pay_frequency,account_id,income_category_id,pay_period_start,pay_period_end,payment_date,base_pay,gross_pay,total_deductions,net_pay,notes,transaction_id,salary_run_components(id,source_profile_component_id,name,component_kind,calculation_type,fixed_amount,percentage,hours,hourly_rate,multiplier,calculated_amount,display_order)",
    )
